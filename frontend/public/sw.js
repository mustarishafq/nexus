...
self.addEventListener('message', (event) => {
  if (event.data?.type === 'CLIENT_DISPLAY_MODE') {
...
async function openNotificationTarget(notificationData = {}) {
  ...
  // Only ever reuse a window that the page has reported as standalone (installed PWA).
  // A same-origin browser tab must not be focused/navigated here, or the click
  // ends up opening the site in the browser instead of the installed app.
  const standaloneClients = sameOriginClients.filter((client) => standaloneClientIds.has(client.id));
  const focusedClient = standaloneClients.find((client) => client.focused);
  const originClient = focusedClient || standaloneClients[0];
