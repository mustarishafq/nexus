...
import {
  invalidateApplicationReleaseNoteQueries,
} from '@/hooks/useApplicationReleaseNotes';
...
export default function ApplicationWhatsNewSheet({
  application,
  open,
  onOpenChange,
  canManage = false,
  ...
  const { data: notes = [], isLoading } = useQuery({
    queryKey: ['application-release-notes', applicationId, canManage],
    queryFn: () =>
      db.entities.ApplicationReleaseNote.filter(
  ...
  const handleOpenChange = (nextOpen) => {
    if (!nextOpen && applicationId) {
      db.markApplicationReleaseNotesRead({ application_id: applicationId })
        .then(() => invalidateApplicationReleaseNoteQueries(queryClient, applicationId))
        .catch(() => {});
    }
  ...
  const saveMut = useMutation({
    mutationFn: async (payload) => {
      if (editingNote?.id) {
        return db.entities.ApplicationReleaseNote.update(editingNote.id, payload);
      }
      return db.entities.ApplicationReleaseNote.create({
        application_id: applicationId,
        ...payload,
      });
    },
    onSuccess: () => {
      invalidateApplicationReleaseNoteQueries(queryClient, applicationId);
      setForm(EMPTY_FORM);
      setEditingNote(null);
  ...
  const deleteMut = useMutation({
    mutationFn: (noteId) => db.entities.ApplicationReleaseNote.delete(noteId),
    onSuccess: () => {
      invalidateApplicationReleaseNoteQueries(queryClient, applicationId);
      setPendingDelete(null);
        ...
        >
          <SheetHeader className="border-b border-border/50 px-6 py-5 text-left">
            ...
            <SheetDescription className={glassDialogMutedText}>
              {application?.name
                ? `Release notes for ${application.name}`
                : 'Release notes for this application'}
            </SheetDescription>
            {canManage && (
              <div className="pt-2">
                <Button
          ...
          <div className="flex-1 space-y-4 overflow-y-auto px-6 py-4">
            {canManage && managing && (
              ...
              >
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="release-note-title">Title</Label>
                  <Input
                    id="release-note-title"
                    value={form.title}
                    onChange={(e) => setForm((prev) => ({ ...prev, title: e.target.value }))}
                    placeholder="New leave export"
                    maxLength={255}
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="release-note-body">Details</Label>
                  <Textarea
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <Label htmlFor="release-note-version">Version</Label>
                    <Input
                ...
                <div className={cn('flex items-center justify-between rounded-lg border px-3 py-2', glassPanelStyles)}>
                  <div>
                    ...
                    <p className={cn('text-xs', glassDialogMutedText)}>Visible to users with app access</p>
                  </div>
                  <Switch
            ...
            {isLoading ? (
              ...
              <div className={cn('rounded-xl border border-dashed px-4 py-10 text-center text-sm', glassDialogMutedText)}>
                {canManage
                  ? 'No release notes yet. Add the first update for this application.'
                  : 'No release notes yet for this application.'}
              </div>
            ) : (
