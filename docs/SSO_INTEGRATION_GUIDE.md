...
3. **Read identity claims**:
   ...
   | `name`  | string | User's SSO **display name** (`users.name` in Nexus). **Omitted** when the user launches with an additional approved SSO email — only `email` is sent so your app can sign in without overwriting the local account profile. |
   | `full_name` | string | Formal / full name (`users.full_name`). **Omitted** when empty — Nexus does not invent a fallback — or when the user launches with an additional SSO email. |
   | `department` | string | Department display name (`departments.name` via `users.department_id`). **Omitted** when the user has no department, or when launching with an additional SSO email. |
...
**Additional-email launches** (approved alternate SSO email) omit **all** profile claims:
no `name`, `full_name`, `department`, or `profile_picture` — only `email` (and standard claims) so your app can sign in without overwriting the local account profile.
